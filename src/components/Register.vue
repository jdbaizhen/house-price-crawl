			 ...
			 <span  class="glyphicon glyphicon-remove close"  @click="showModal=false"></span>
	          ...
	          <div class="modal-body">
	            <slot name="body">
			    	...
			    	<div class="form-group">
			    		<input type="text" class="form-control" name="userPassword" placeholder="密码" v-model="pwd">
			    	</div>
			    	<button type="submit" class="btn btn-success" @click="submit">提&nbsp;&nbsp;&nbsp;&nbsp;交</button>
	            </slot>
	         </div>
	        </div>
	      </div>
...
export default {
  ...
  methods : {
  	//后台注册接口
  	submit :　function(){
  		const pwd = md5(this.pwd);	//对密码进行加密
  		const self = this;
		this.$http.post('/registerNumber',{
